...
exports.getUserResumes = async (req, res) => {
  try {
    ...
    // Return empty array if no resumes found instead of 404
    ...
    // Process the resumes to ensure all required fields are present
    const processedResumes = resumes.map(resume => ({
      ...resume,
      history: resume.history || [],
      personalInfo: resume.personalInfo || {},
      templateId: resume.templateId || '1', // Default template
...
exports.uploadToGoogleDocs = async (req, res) => {
  try {
    ...
    // Save the Google Doc reference in the user's resume
    if (req.user?._id) {
